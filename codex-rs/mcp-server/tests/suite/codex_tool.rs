...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Build a cross‑platform command that creates a file without relying on Python.
    // - Windows: use PowerShell to create the file
    // - Unix:    use `sh -c ": > <file>"` (shell builtin redirection)
    let shell_command = if cfg!(windows) {
        vec![
            ...
            "-Command".to_string(),
            format!("New-Item -ItemType File -Path {created_filename}"),
        ]
    } else {
        vec![
            "sh".to_string(),
