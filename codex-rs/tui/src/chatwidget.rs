...
impl ChatWidget {
    ...
    fn on_interrupted_turn(&mut self) {
        ...
        self.finalize_turn_with_error_message("您中止了模型，您希望它改做什么".to_owned());
                    ...
                    let text = match get_git_diff().await {
                        Ok((is_git_repo, diff_text)) => {
                            if is_git_repo {
                                ...
                                "`/diff` — 当前目录不是一个 git 仓库".to_string()
                            }
                        }
    ...
    pub(crate) fn open_model_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(
            "选择模型和推理级别".to_string(),
            Some("在本次及未来的 Codex CLI 会话中切换不同的 OpenAI 模型".to_string()),
            Some("按 Enter 确认，或按 Esc 返回".to_string()),
            items,
        );
    }
