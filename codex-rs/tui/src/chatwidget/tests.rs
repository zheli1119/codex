...
async fn binary_size_transcript_snapshot() {
    ...
    // Anchor at the banner line beginning with a distinctive ">_ " prompt,
    // independent of language; the banner always starts with ">_ ".
    let last_marker_line_idx = lines
        .iter()
        .rposition(|l| l.trim_start().starts_with(">_ "))
...
fn plan_update_renders_history_cell() {
    ...
    let blob = lines_to_single_string(cells.last().unwrap());
    assert!(
        blob.contains("Updated Plan") || blob.contains("计划已更新"),
