...
fn display_path_for(path: &Path, cwd: &Path) -> String {
    // 1) Prefer paths inside current working directory without introducing ".."
    if let Ok(rel) = path.strip_prefix(cwd) {
        return rel.display().to_string();
    }

    // 2) If both paths are in the same Git repo, fall back to repo-relative diff
    let path_in_same_repo = match (get_git_repo_root(cwd), get_git_repo_root(path)) {
        (Some(cwd_repo), Some(path_repo)) => cwd_repo == path_repo,
        _ => false,
    };
    if path_in_same_repo {
        return pathdiff::diff_paths(path, cwd)
            .unwrap_or_else(|| path.to_path_buf())
            .display()
            .to_string();
    }

    // 3) Else try to relativize to HOME
    if let Some(home_rel) = relativize_to_home(path) {
        return home_rel.display().to_string();
    }

    // 4) Fallback: absolute or original path
