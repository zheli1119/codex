...
pub(crate) fn new_session_info(
    ...
    if is_first_event {
        ...
        lines.push(Line::from(vec![
            ">_ ".dim(),
            "您正在使用 OpenAI Codex, 当前处于".bold(),
            format!(" {cwd_str}").dim(),
        ]));
        lines.push(Line::from("".dim()));
        lines.push(Line::from(
            " 会话开始!可以描述一个任务，或者尝试以下命令之一:".dim(),
        ));
        lines.push(Line::from("".dim()));
        if !has_agents_md {
            lines.push(Line::from(vec![
...
pub(crate) fn new_status_output(
    ...
    let sandbox_name = match &config.sandbox_policy {
        SandboxPolicy::DangerFullAccess => "完全访问（危险）",
        SandboxPolicy::ReadOnly => "只读",
        SandboxPolicy::WorkspaceWrite { .. } => "仅限工作区写入",
    };
    lines.push(vec!["  • 沙盒模式: ".into(), sandbox_name.into()].into());
    ...
    {
        lines.push(vec![padded_emoji("👤").into(), "账号信息".bold()].into());
        lines.push("  • 已通过 ChatGPT 登录".into());

        let info = tokens.id_token;
        if let Some(email) = &info.email {
            lines.push(vec!["  • 登录邮箱: ".into(), email.clone().into()].into());
        }

        match auth.openai_api_key.as_deref() {
            Some(key) if !key.is_empty() => {
                lines.push(
                    "  • 正在使用 API Key。运行 codex login 以切换到 ChatGPT 订阅计划".into(),
                );
            }
            _ => {
                let plan_text = info
                    ...
                    .map(|s| title_case(&s))
                    .unwrap_or_else(|| "未知".to_string());
                lines.push(vec!["  • 订阅计划: ".into(), plan_text.into()].into());
            }
        }
    ...
    lines.push(vec!["  • 模型名称: ".into(), config.model.clone().into()].into());
    let provider_disp = pretty_provider_name(&config.model_provider_id);
    lines.push(vec!["  • 供应商: ".into(), provider_disp.into()].into());
    ...
    if !rsum.is_empty() {
        lines.push(vec!["  • 推理总结: ".into(), title_case(&rsum).into()].into());
    }

    lines.push("".into());

    // 📊 Token Usage
    lines.push(vec!["📊 ".into(), "Token 使用情况".bold()].into());
    if let Some(session_id) = session_id {
        lines.push(vec!["  • 会话 ID: ".into(), session_id.to_string().into()].into());
    }
    ...
    if let Some(cached) = usage.cached_input_tokens
        && cached > 0
    {
        input_line_spans.push(format!(" (+ {cached} 缓存)").into());
    }
...
impl HistoryCell for PlanUpdateCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let mut lines: Vec<Line<'static>> = vec![];
        lines.push(vec!["• ".into(), "计划已更新".bold()].into());
        ...
        if self.plan.is_empty() {
            indented_lines.push(Line::from("(未提供任何步骤)".dim().italic()));
        } else {
...
pub(crate) fn new_patch_apply_failure(stderr: String) -> PlainHistoryCell {
    ...
    // Failure title
    lines.push(Line::from("✘ 修改失败".magenta().bold()));
...
pub(crate) fn new_proposed_command(command: &[String]) -> PlainHistoryCell {
    ...
    let mut lines: Vec<Line<'static>> = Vec::new();
    lines.push(Line::from(vec!["• ".into(), "建议的命令".bold()]));
