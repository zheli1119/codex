...
impl AuthModeWidget {
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "使用 ChatGPT 登录，在您的付费计划中使用 Codex".bold(),
            ]),
            Line::from(vec!["  ".into(), "或 使用API key 登录".bold()]),
            "".into(),
        ];
    ...
    fn render_continue_in_browser(&self, area: Rect, buf: &mut Buffer) {
        ...
        {
            lines.push("  如果链接未自动打开，请打开以下链接进行认证：".into());
    ...
    fn render_chatgpt_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ 已使用您的 ChatGPT 账号登录".fg(Color::Green).into(),
            "".into(),
            "> 在开始之前：".into(),
            "".into(),
            ...
            "".into(),
            "  Codex 可能会出错".into(),
            "  请审查其编写的代码和执行的命令".dim().into(),
            "".into(),
            "  由您的 ChatGPT 账户提供支持".into(),
            Line::from(vec![
                "  将使用您套餐的速率限制与 ".into(),
                "\u{1b}]8;;https://chatgpt.com/#settings\u{7}训练数据偏好\u{1b}]8;;\u{7}"
                    .underlined(),
            ])
            .dim(),
            "".into(),
    ...
    fn render_chatgpt_success(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec!["✓ 已通过 ChatGPT 帐号登录".fg(Color::Green).into()];
    ...
    fn render_env_var_found(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec!["✓ 已检测到 OPENAI_API_KEY".fg(Color::Green).into()];
    ...
    fn render_env_var_missing(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "  如需在 OpenAI API 下使用 Codex，请在环境中设置 OPENAI_API_KEY"
                .fg(Color::Cyan)
                .into(),
            "".into(),
            "  按 Enter 返回".dim().into(),
