...
static COMMAND_SELECT_OPTIONS: LazyLock<Vec<SelectOption>> = LazyLock::new(|| {
    vec![
        SelectOption {
            label: Line::from("是"),
            description: "批准并运行该命令",
            key: KeyCode::Char('y'),
            decision: ReviewDecision::Approved,
        },
        SelectOption {
            label: Line::from("始终允许"),
            description: "在本次会话期间始终批准此命令",
            key: KeyCode::Char('a'),
            decision: ReviewDecision::ApprovedForSession,
        },
        SelectOption {
            label: Line::from("否，提供反馈"),
            description: "不运行该命令；提供反馈",
            key: KeyCode::Char('n'),
...
impl WidgetRef for &UserApprovalWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        let title = match &self.approval_request {
            ApprovalRequest::Exec { .. } => "允许运行命令？",
            ApprovalRequest::ApplyPatch { .. } => "应用更改？",
        };
